...
<div v-else class="layout">
  ...
  <aside class="sidebar" :class="{open: sidebarOpen}">
    <div class="brand"><span class="logo"><svg xmlns="http://www.w3.org/2000/svg" viewBox="76 309 1848 1382"><path fill="#400099" d="M1859.38,594.85c-27.08-48.82-67.97-88.24-122.68-118.26-54.73-30.02-121.53-45.03-200.37-45.03h-342.7c53.85,70.05,81.07,157.67,81.07,261.49,0,4.08-.03,8.13-.09,12.14h205.25c29.42,0,51.77,6.18,67.07,18.55,15.29,12.37,22.94,30.3,22.94,53.83,0,4.71-1.17,14.14-3.52,28.25-5.88,31.77-20.02,56.48-42.39,74.13-22.37,17.65-51.19,26.48-86.5,26.48h-198.27c-.69,2.08-1.37,4.19-2.07,6.25-18.69,54.42-43.68,102.78-74.29,143.74-38.66,51.74-87.23,95.66-144.87,131.08l47.42,179.43,40.21-188.65h291.28c29.42,0,52.96,7.65,70.61,22.96,17.65,15.31,26.48,35.31,26.48,60.02,0,36.5-13.84,67.69-41.49,93.58-27.68,25.88-61.51,38.82-101.51,38.82h-278.52l49.96,189.04c3.2,9.76,4.81,20.22,4.81,31.21,0,18.76-3.89,36.77-11.37,53.37h231.6c105.92,0,190.95-16.18,255.08-48.55,64.13-32.36,111.51-74.73,142.1-127.09,30.59-52.36,54.13-115.64,70.64-189.8,7.03-34.11,10.57-64.71,10.57-91.78,0-69.42-26.48-125.92-79.44-169.48,28.25-18.83,56.8-46.18,85.62-82.08,28.82-35.9,49.14-83.28,60.91-142.12,4.69-18.8,7.06-41.17,7.06-67.07,0-54.13-13.54-105.62-40.62-154.47Z"/><path fill="#e56b00" d="M1039.9,1651.1c9.17-10.8,13.77-23.19,13.77-37.17,0-3.82-.55-6.99-1.61-9.54l-119.92-453.77c71.3-34.3,128.56-80.38,171.78-138.22,26.17-35.02,47.37-76.23,63.64-123.6,10.59-30.86,19.1-64.32,25.5-100.42,3.17-15.74,5.41-33.27,6.72-52.55.91-13.41,1.37-27.66,1.37-42.78,0-111.7-36.08-198.85-108.2-261.49-1.73-1.5-3.46-3.01-5.24-4.48-75.63-62.91-171.78-94.38-288.46-94.38h-423c-11.88,0-22.96,5.1-33.22,15.26-10.27,10.16-16.48,22.25-18.64,36.23L100.76,1615.84c-2.16,13.98.25,26.07,7.29,36.23,7.03,10.16,16.48,15.24,28.37,15.24h212.3c11.86,0,22.94-5.08,33.22-15.24,10.25-10.16,16.48-22.25,18.64-36.23l74.54-408.03h165.29l72.78,281.91,32.56,126.13c4.32,15.26,10.53,27.65,18.64,37.17,8.09,9.54,21.88,14.3,41.33,14.3h204.19c10.8,0,20.78-5.4,29.97-16.21ZM894.85,769.3c-7.63,41.52-23.81,73.87-48.49,97.06-31.24,29.34-76.11,44.03-134.64,44.03h-183.11l50.23-282.16h183.13c63.72,0,105.03,17.57,124.01,52.65,8.05,14.88,12.11,32.9,12.11,54.1,0,5.1-1.08,16.53-3.24,34.32Z"/></svg></span><span>RegiBase</span><span class="tag" v-if="version">v{{ version }}</span></div>
      ...
      <template v-else>
        ...
        <template v-else>
          ...
          <div v-else-if="curView==='note'" class="note-view">
            ...
            <div class="note-pane note-content" :style="{'--rb-coll-color': current.color}">
  ...
  <div v-if="modal && modal.type==='secretReveal'" class="modal-mask" @click.self="modal=null">
    <div class="modal sm secret-modal">
      <div class="modal-head"><h3>{{ t('🕶️ Secret toggle') }}</h3><button class="icon-btn" @click="modal=null">✕</button></div>
      <div class="modal-body">
        <p class="secret-lead">{{ t('Enter the 6-digit secret key to show its collections.') }}</p>
        <div class="secret-cells">
          <input v-for="(n, i) in 6" :key="i" ref="cells" class="secret-cell" type="password" inputmode="numeric" autocomplete="off" autocorrect="off" spellcheck="false" data-1p-ignore data-lpignore="true"
            maxlength="1" :value="secretForm.cells[i]"
            @focus="$event.target.select()"
            @input="onSecretCellInput(i, $event)"
            @keydown="onSecretCellKey(i, $event)"
            @paste="onSecretPaste" />
        </div>
  ...
  <template v-if="pwgen.open">
    ...
    <div class="pwgen-popup" @click.stop>
      ...
      <div v-if="pwgenIsHex" class="pwgen-opts">
        <div class="pwgen-note">{{ t('Hexadecimal field: fixed 0–9 A–F alphabet.') }}</div>
      </div>
      <template v-else>
        <div class="pwgen-classes">
          <div class="pwgen-hdr"><span>{{ t('Character types') }}</span><span class="pwgen-mmhdr">{{ t('min') }} / {{ t('max') }}</span></div>
          <div class="pwgen-cls" v-for="c in pwgenClassRows" :key="c.k" :class="{dis: !c.avail}">
            <label class="pwgen-cls-on">
              <input type="checkbox" :checked="pwgen[c.k]" :disabled="!c.avail" @change="pwgenToggle(c.k, $event)" /> {{ t(c.label) }}
            </label>
            <span class="pwgen-mm" v-if="c.avail && pwgen[c.k] && c.set.length">
              <input type="number" min="0" :max="pwgenMax" :value="pwgen.min[c.k]" @change="pwgenSetMin(c.k, $event.target.value)" :title="t('Minimum count')" />
              <span class="pwgen-mm-sep">/</span>
              <input type="number" min="0" :max="pwgenMax" :value="pwgen.max[c.k] == null ? '' : pwgen.max[c.k]" :placeholder="t('∞')" @change="pwgenSetMax(c.k, $event.target.value)" :title="t('Maximum count (blank = no limit)')" />
            </span>
          </div>
        </div>
        <div v-if="pwgenSymbolsAvail && pwgen.symbols" class="pwgen-symsel">
          <div class="pwgen-symsel-head">
            <span class="sub">{{ t('Allowed symbols') }}</span>
            <button type="button" class="pwgen-mini" @click="pwgenAllSymbols(true)">{{ t('All') }}</button>
            <button type="button" class="pwgen-mini" @click="pwgenAllSymbols(false)">{{ t('None') }}</button>
          </div>
          <div class="pwgen-symgrid">
            <button type="button" class="pwgen-sym" v-for="s in pwgenSymbolChips" :key="s.ch"
                    :class="{sel: s.on, dis: s.lookalike && pwgen.noLookalike}"
                    :title="s.lookalike && pwgen.noLookalike ? t('Excluded as a look-alike') : ''"
                    @click="pwgenToggleSymbol(s.ch)">{{ s.ch }}</button>
          </div>
        </div>
